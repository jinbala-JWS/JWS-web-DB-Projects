...
| [register_daily_task.ps1](./register_daily_task.ps1) | Windows 작업 스케줄러에 매일 08:00 실행되도록 등록(재현용) |
...
- Windows 작업 스케줄러(`CPI_DailyPriceCollector`, 매일 08:00, 로그인 사용자 계정으로 실행 —
...
- 오피넷 폼 파라미터(POST 필드명)는 브라우저 개발자도구 없이 `new FormData(document.forms['form1'])`로
  ...
  추가할 때도 동일한 방법 사용 가능.
- 재등록/시간 변경: `register_daily_task.ps1`의 `-At 08:00` 수정 후 재실행.
