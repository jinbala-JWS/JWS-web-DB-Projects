...
- 연 1회 정해진 달에 계단식으로 바뀌는 10개 품목(진료비 5종=매년1월, 등록금 5종=매년3월,
  전부 Tier C)은 계절ETS 적용(연 1회 패턴도 "계절성"으로 근사)
- Tier C 나머지 21개 품목은 비계절 ETS
- 휴대전화기(Tier A)는 갤럭시/아이폰 신모델 출시 패턴을 근거로 계절ETS 편입을 시도했으나
  MAPE가 0.47%->3.07%로 악화되어 되돌림(ANNUAL_STEP_NAMES 주석 참조) — 비계절ETS 유지
- Tier D:
...
# 앞 12개월(2024-08~2025-07)은 번인(burn-in) 구간 - 항목별 편향보정 계수를
# 미래정보 누출 없이 추정하기 위한 트레일링 잔차용. 실제 검증 대상은 뒤 12개월.
BURNIN_MONTHS = [
...
TARGET_MONTHS = BURNIN_MONTHS + TEST_MONTHS
...
# 2026-08-24 시도: 휴대전화기(Tier A)를 갤럭시/아이폰 신모델 출시 패턴 근거로
# ANNUAL_STEP_NAMES(계절ETS)에 편입해봤으나 MAPE 0.47%->3.07%로 크게 악화되어 되돌림.
# 원인 추정: 진료비·등록금과 달리 "휴대전화기" CPI 품목은 특정 두 브랜드가 아니라
# 전체 브랜드/모델을 아우르는 지수라, 26년 히스토리에 걸쳐 매년 같은 달에 오르는
# 깨끗한 계절패턴이 실제로는 없음(과거엔 신모델 출시 반영이 지금처럼 뚜렷하지
# 않았을 가능성) - 계절ETS가 존재하지 않는 패턴을 억지로 학습해 과적합됨.
# -> 이 품목은 비계절ETS + 게이트 기반 편향보정(항목별_편향보정_결과.md) 조합을 유지.
...
def main():
    ...
    for i, row in panel.iterrows():
        ...
        elif item in ANNUAL_STEP_NAMES:
            is_seasonal = True
            restrict_2017 = False
        else:
            is_seasonal = False
